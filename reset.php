...
if (!isset($_POST['password'])) {
    $SQLstring = "select email from tokens where token='" . $token . "' and used=0";
...
if (!isset($pass)) {
    // CHANGE: add token to query string for form
    echo '<form method="POST" action="reset.php?token=' . htmlspecialchars(urlencode($token)) . '">
...
if (isset($_POST['password']) && isset($_SESSION['email'])) {
    ...
    $SQLstring = "update members set password_hash='$hash', password_salt='$salt' where email='" . $email . "'";
    $QueryResult = mysql_query($SQLstring);
    if ($QueryResult) {
